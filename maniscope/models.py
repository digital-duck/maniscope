...
- Manifold Ranking — Zhou et al. (NIPS 2003)   [TMLR revision baseline #15]
- Diffusion-Aided RAG — Dampanaboina et al. (CLiC-it 2025)  [TMLR revision baseline #16]
- Donoser Diffusion (PSP) — Donoser & Bischof (CVPR 2013)   [TMLR revision baseline #19]
"""
...
def load_maniscope_reranker(k: int = 5, alpha: float = 0.3, version: Optional[str] = None, embedding_model: str = 'all-MiniLM-L6-v2'):
    ...
    try:
        ...
        if version == 'v2o':
            ...
            from maniscope_engine import ManiscopeEngine_v1
            ...
            from maniscope_engine import ManiscopeEngine_v2
            ...
            from maniscope_engine import ManiscopeEngine_v3
            ...
            from maniscope_engine import ManiscopeEngine
...
@st.cache_resource
def load_manifold_ranking_reranker(
    k: int = 5,
    alpha: float = 0.99,
    embedding_model: str = 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2'
) -> Any:
    """
    Load Manifold Ranking reranker (Zhou et al., NIPS 2003).

    Args:
        k: k-NN graph connectivity
        alpha: Propagation weight (0.99 = strong smoothing)
        embedding_model: Sentence-Transformers model name

    Returns:
        ManifoldRankingEngine instance
    """
    try:
        from manifold_ranking_engine import ManifoldRankingEngine
        model = ManifoldRankingEngine(
            model_name=embedding_model,
            k=k,
            alpha=alpha,
            verbose=False
        )
        print(f"✓ Loaded Manifold Ranking reranker (k={k}, α={alpha})")
        return model
    except Exception as e:
        raise RerankerModelError(f"Failed to load Manifold Ranking model: {e}")


@st.cache_resource
def load_diffusion_rag_reranker(
    k: int = 5,
    alpha: float = 0.95,
    embedding_model: str = 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2'
) -> Any:
    """
    Load Diffusion-Aided RAG reranker (Dampanaboina et al., CLiC-it 2025).

    Args:
        k: k-NN graph connectivity
        alpha: Diffusion restart complement (0.95 = strong diffusion)
        embedding_model: Sentence-Transformers model name

    Returns:
        DiffusionRAGEngine instance
    """
    try:
        from diffusion_rag_engine import DiffusionRAGEngine
        model = DiffusionRAGEngine(
            model_name=embedding_model,
            k=k,
            alpha=alpha,
            verbose=False
        )
        print(f"✓ Loaded Diffusion-Aided RAG reranker (k={k}, α={alpha})")
        return model
    except Exception as e:
        raise RerankerModelError(f"Failed to load Diffusion-Aided RAG model: {e}")


@st.cache_resource
def load_donoser_diffusion_reranker(
    k: int = 5,
    alpha: float = 0.99,
    embedding_model: str = 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2'
) -> Any:
    """
    Load Donoser Diffusion (PSP) reranker — Donoser & Bischof (CVPR 2013).

    Args:
        k: k-NN graph connectivity (before mutual filtering)
        alpha: Diffusion weight (0.99 = strong graph smoothing)
        embedding_model: Sentence-Transformers model name

    Returns:
        DonoserDiffusionEngine instance
    """
    try:
        from donoser_diffusion_engine import DonoserDiffusionEngine
        model = DonoserDiffusionEngine(
            model_name=embedding_model,
            k=k,
            alpha=alpha,
            verbose=False
        )
        print(f"✓ Loaded Donoser Diffusion reranker (k={k}, α={alpha})")
        return model
    except Exception as e:
        raise RerankerModelError(f"Failed to load Donoser Diffusion model: {e}")
...
def load_all_models(selected_models: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    for model_name in selected_models:
        try:
            ...
            elif model_name == "Manifold Ranking":
                k = st.session_state.get('maniscope_k', 5)
                embedding_model = st.session_state.get('hnsw_embedding_model', 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2')
                models[model_name] = load_manifold_ranking_reranker(k=k, embedding_model=embedding_model)
            elif model_name == "Diffusion-Aided RAG":
                k = st.session_state.get('maniscope_k', 5)
                embedding_model = st.session_state.get('hnsw_embedding_model', 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2')
                models[model_name] = load_diffusion_rag_reranker(k=k, embedding_model=embedding_model)
            elif model_name == "Donoser Diffusion":
                k = st.session_state.get('maniscope_k', 5)
                embedding_model = st.session_state.get('hnsw_embedding_model', 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2')
                models[model_name] = load_donoser_diffusion_reranker(k=k, embedding_model=embedding_model)
            else:
...
def run_manifold_ranking_reranker(model: Any, query: str, docs: List[str]) -> np.ndarray:
    """
    Run Manifold Ranking reranker (Zhou et al., NIPS 2003).

    Args:
        model: ManifoldRankingEngine instance
        query: Query text
        docs: List of document texts

    Returns:
        Numpy array of relevance scores (one per document)
    """
    try:
        return model.rerank(query, docs)
    except Exception as e:
        raise RerankerModelError(f"Manifold Ranking reranker failed: {e}")


def run_diffusion_rag_reranker(model: Any, query: str, docs: List[str]) -> np.ndarray:
    """
    Run Diffusion-Aided RAG reranker (Dampanaboina et al., CLiC-it 2025).

    Args:
        model: DiffusionRAGEngine instance
        query: Query text
        docs: List of document texts
    ...
    try:
        ...
        raise RerankerModelError(f"Diffusion-Aided RAG reranker failed: {e}")


def run_donoser_diffusion_reranker(model: Any, query: str, docs: List[str]) -> np.ndarray:
    """
    Run Donoser Diffusion (PSP) reranker — Donoser & Bischof (CVPR 2013).

    Args:
        model: DonoserDiffusionEngine instance
        query: Query text
        docs: List of document texts
    ...
    try:
        ...
        raise RerankerModelError(f"Donoser Diffusion reranker failed: {e}")
...
def run_reranker(model: Any, query: str, docs: List[str]) -> np.ndarray:
    ...
    try:
        ...
        elif "ManifoldRankingEngine" in model_class:
            return run_manifold_ranking_reranker(model, query, docs)
        elif "DiffusionRAGEngine" in model_class:
            return run_diffusion_rag_reranker(model, query, docs)
        elif "DonoserDiffusionEngine" in model_class:
            return run_donoser_diffusion_reranker(model, query, docs)
