...
def calculate_mrr(rankings: List[int], relevance_map: Dict[str, int], k: Optional[int] = None) -> float:
    """
    Calculate Mean Reciprocal Rank (optionally truncated at rank k).

    MRR@k = 1 / rank_of_first_relevant_doc  (if rank <= k, else 0)
    ...
    Args:
        ...
        k: Rank cutoff (None = no cutoff, i.e. consider all positions)

    Returns:
        MRR score (0.0 to 1.0). Returns 0.0 if no relevant docs found within k.
    ...
    Example:
        ...
        >>> calculate_mrr(rankings, relevance_map, k=10)
        0.5  # Same result when k >= position of first relevant doc
    """
    ranked = rankings[:k] if k is not None else rankings
...
def calculate_dcg(rankings: List[int], relevance_map: Dict[str, int], k: Optional[int] = None) -> float:
    """
...
def calculate_ndcg(rankings: List[int], relevance_map: Dict[str, int], k: Optional[int] = None) -> float:
    """
...
def calculate_all_metrics(rankings: List[int], relevance_map: Dict[str, int]) -> Dict[str, float]:
    ...
    return {
        ...
        "MRR@10": calculate_mrr(rankings, relevance_map, k=10),
