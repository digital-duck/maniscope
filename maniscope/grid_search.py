...
Usage:
    python grid_search.py --dataset aorb --k-range 3:15:1 --alpha-range 0.0:1.0:0.25
    python grid_search.py --dataset scifact --k-range 5:15:1 --alpha-range 0.0:0.5:0.1


✓ Completed:
- Added P@3, P@5 (also NDCG@5)
- Created jupyter notebook with 3D contour plots (k,alpha) vs each metric
- Notebook supports multi-dataset comparison
"""
...
def load_dataset(dataset_name: str) -> Tuple[List[dict], Path]:
    ...
    dataset_map = {
        ...
        'nfcorpus': 'dataset-nfcorpus.json',
        'nfcorpus-10': 'dataset-nfcorpus-10.json',
        'fever': 'dataset-fever.json',
        'scidocs': 'dataset-scidocs.json',
        'hotpotqa': 'dataset-hotpotqa.json',
        'nq': 'dataset-nq.json',
        'quora': 'dataset-quora.json',
        'dbpedia': 'dataset-dbpedia.json',
        'climate-fever': 'dataset-climate-fever.json',
        'touche2020': 'dataset-touche2020.json',
    }
...
def grid_search(dataset: str, k_range: str, alpha_range: str,
    ...
    if 'MRR' in df.columns:
        ...
        if 'NDCG@5' in df.columns:
            click.echo(f"    NDCG@5:  {best_result['NDCG@5']:.4f}")
        if 'NDCG@10' in df.columns:
            click.echo(f"    NDCG@10: {best_result['NDCG@10']:.4f}")
        if 'P@3' in df.columns:
            click.echo(f"    P@3:     {best_result['P@3']:.4f}")
        if 'P@5' in df.columns:
            click.echo(f"    P@5:     {best_result['P@5']:.4f}")
        ...
        cols = ['k', 'alpha', 'MRR', 'NDCG@3', 'NDCG@5', 'P@3', 'P@5', 'MAP']
        available_cols = [c for c in cols if c in df.columns]
        top5 = df.nlargest(5, 'MRR')[available_cols]
